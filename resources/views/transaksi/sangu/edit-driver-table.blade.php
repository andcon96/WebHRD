<table class="table table-bordered drivertable" id="driverTable" width="100%" cellspacing="0">
    ...
    <tbody id="driverbody">
        @forelse($data->getSangu as $key => $datas)
            <tr>
                <td>
                    {{$datas->getTruckDriver->getTruck->truck_no_polis}} -- {{$datas->getTruckDriver->getUser->name}}
                    <input type="hidden" value="{{$datas->sos_truck}}" name="polis[]"/>
                </td>
                <td>
                    <input type="number" value="{{$datas->sos_tot_trip}}" class="form-control" name="qtytrip[]"/>
                </td>
                <td>
                    <input type="text" value="{{number_format($datas->sos_sangu,0)}}" class="form-control sangu" name="totsangu[]"/>
                </td>
